...
dfnad['genres'] = dfnad['genres'].apply(ast.literal_eval)
dfnad = dfnad.explode('genres')
dfnad.to_csv("again_dfnad.csv")
#df_exploded = dfnad['genres'].explode()
#df_exploded.to_csv("genres_exploded.csv")

genre_imdb_scores = dfnad.groupby('genres')['imdb_score'].mean().sort_values(ascending=False)

genre_df = genre_imdb_scores.reset_index()
genre_imdb_scores.to_csv("genres_imdb_scores.csv")

sea.barplot(data=genre_df, y='genres', x='imdb_score', palette='Pastel2')
#plt.show()
